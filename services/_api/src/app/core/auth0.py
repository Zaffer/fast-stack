...
auth0_rule_namespace: str = os.getenv("AUTH0_RULE_NAMESPACE", "https://your.domain.com")
    ...
    async def get_user(
        ...
        try:
            ...
            if self.email_auto_error and not user.email:
                raise Auth0UnauthorizedException(
                    detail='Missing email claim (check rule "Add email to access token")'
                )

            return user

        except ValidationError as e:
            logger.error(f'Handled exception parsing auth user: "{e}"', exc_info=True)
            if self.auto_error:
                raise Auth0UnauthorizedException(detail="Error parsing auth user")
            else:
                return None
