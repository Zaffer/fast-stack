 ...
 */
export const onSummaryCreated = onDocumentCreated("summaries/{docId}", async (event) => {
    ...
    try {
        const responseSend = await admin.messaging()
...
*/
export const onThemeFollowed = onDocumentUpdated('users/{userId}/portfolios/{themeId}', async (event) => {
    ...
    if (newValue.followed !== previousValue.followed) {
        console.log(`Theme '${event.params.themeId}' followed status changed to: ${newValue.followed}`);

        if (newValue.followed === true) {
            // get all companies that have the theme being followed
            const companiesRef = db.collection('companies').where('follow_suggestions.themes', 'array-contains', event.params.themeId);
            ...
            companiesSnapshot.forEach(async (doc) => {
                ...
                const followingsDoc = db.collection('followings').doc(`${doc.id}|${event.params.userId}`);
 ...
 */
export const onNotificationsToggled = onDocumentUpdated('followings/{docId}', async (event) => {
    ...
    if (newData.is_notified === true) {
        ...
        try {
            const responseUnsubscribe = await admin.messaging()
                .unsubscribeFromTopic(registrationTokens, newData.company_id)
            console.log('successfully unsubscribed from topic:', responseUnsubscribe);
            return null
        }
        catch (error) {
